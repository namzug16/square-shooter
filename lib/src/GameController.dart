...
class GameController extends StateNotifier<bool>{
  ...
  void renderGame(Canvas c, x){
    ...
    if(_playerController.isHurt){
      c.translate(_hurtValues[Random().nextInt(_hurtValues.length)], _hurtValues[Random().nextInt(_hurtValues.length)]);
    }
    if((!_playerController.isAlive() || !_enemyController.isAlive()) && _deathShakes < 20){
      c.translate(_deathValues[Random().nextInt(_deathValues.length)], _deathValues[Random().nextInt(_deathValues.length)]);
      _deathShakes++;
    }
    _playerController.renderPlayer(c, x);
    // _playerController.renderHitBox(c);
